function autoInicioRelacionCliente(){
    ...
    $.ajax({
        url:"http://129.151.111.95:8080/api/Client/all",
        type:"GET",
        datatype:"JSON",
...
function autoInicioDoctor(){
    ...
    $.ajax({
        url:"http://129.151.111.95:8080/api/Skate/all",
        type:"GET",
        datatype:"JSON",
        success:function(respuesta){
...
function agregarReservation() {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            url:"http://129.151.111.95:8080/api/Reservation/save",
            data: dataToSend,
            datatype: "json",
...
function listarReservation(){
    $.ajax({
        url:"http://129.151.111.95:8080/api/Reservation/all",
        type: "GET",
        datatype: "JSON",
        success: function (response) {
            console.log(response);
    ...
    $.ajax(
        {
            dataType: 'json',
            data: dataToSend,
            url:"http://129.151.111.95:8080/api/Reservation/"+idElemento,
            type: 'DELETE',
            contentType: "application/JSON",
            success: function (response) {
                console.log(response);
...
function cargarDatosReservation(id) {
    $.ajax({
        dataType: 'json',
        url:"http://129.151.111.95:8080/api/Reservation/"+id,
        type: 'GET',
...
function actualizarReservation(idElemento) {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            data: dataToSend,
            contentType: "application/JSON",
            url:"http://129.151.111.95:8080/api/Reservation/update",
            type: "PUT",
